...
class Multi_menu {
	 ...
	 * @var string
	 */
	private $item_anchor              = '<a href="%s" data-permission="%s">%s</a>';
    ...
    private function render_item($items, &$html = '')
	{
	    ...
	    {
	    	/*
	    	if($noTienePermisosGrupo)
	    		$noHagoNada->noSeMuestra;
	    	else
	    		$haceLoQueVieneAContinuacion;
	    	*/
	        ...
	        if ( isset($item[$this->menu_label], $item[$this->menu_key]) )
	        {
		        ...
		        $label = '<span>'.$item[$this->menu_label].'</span>';
				...
				if (substr_count($item_anchor, '%s') == 3) {
					$html .= sprintf($item_anchor, $href, $item['seguridad'], $label);
				}
				elseif (substr_count($item_anchor, '%s') == 2) {
					$html .= sprintf($item_anchor, $href, $label);
				}
				else {
					$html .= sprintf($item_anchor, $label);
