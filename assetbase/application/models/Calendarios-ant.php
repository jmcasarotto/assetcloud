...
class Calendarios extends CI_Model
{
	...
	function getbacklog($data = null){
		...
		where tbl_back.estado = 'C' AND month(tbl_back.fecha) = 01 " ;
	...
	/// MANDA ORDENES AL CALENDARIO (programadas)
	function getot($data = null){

		$month = $data['month'] + 1 ;

		$sql= "select orden_trabajo.id_orden,orden_trabajo.nro,orden_trabajo.fecha_program,orden_trabajo.tipo,orden_trabajo.descripcion
		from orden_trabajo
...
// Preventivos para la Tabla
	function getpredlist(){
...
//Guarda orden de trabajo a partir de Correct, Pred
